...
from models.utils import str2bool, Timer, freeze_net_layers, store_labels
from models.SSDLite import MatchPrior
from dataloaders.datasets.voc_dataset import VOCDataset
from datetime import datetime
from models.multibox_loss import MultiboxLoss
from models.utils import create_mobilenetv2_ssd_lite
from hyper_parameters import mobilenetv1_ssd_config
from dataloaders.data_preprocessing import TrainAugmentation, TestTransform


def get_experiment_log_name() -> str:
    return datetime.now().strftime('%Y-%m-%d_%H:%M')


writer = SummaryWriter('runs/' + get_experiment_log_name())
...
parser.add_argument(
    ...
    default="voc",
    type=str,
    help='Specify dataset type. Currently support voc and '
    'open_images.'
)
...
parser.add_argument(
    '--balance_data',
    action='store_true',
    help="Balance training data by down-sampling more "
    "frequent labels."
)

parser.add_argument(
    '--net',
    default="mb2-ssd-lite",
    help="The network architecture, it can be mb1-ssd, "
    "mb1-lite-ssd, mb2-ssd-lite or vgg16-ssd."
)
parser.add_argument(
...
parser.add_argument(
    ...
    default=None,
    type=float,
    help='initial learning rate for the layers not in base '
    'net and prediction heads.'
)
...
parser.add_argument(
    ...
    default="multi-step",
    type=str,
    help="Scheduler for SGD. It can one of multi-step and "
    "cosine"
)
...
def train(loader, net, criterion, optimizer, device, debug_steps=100, epoch=-1):
    ...
    for i, data in tqdm(enumerate(loader),total=len(loader)):
        ...
        if i and i % debug_steps == 0:
            ...
            logging.info(
                f"Epoch: {epoch}, Step: {i}, " + f"Average Loss: {avg_loss:.4f}, " +
                f"Average Regression Loss {avg_reg_loss:.4f}, " +
                f"Average Classification Loss: {avg_clf_loss:.4f}"
            )

            writer.add_scalar('train/loss/average', avg_loss, epoch)
            writer.add_scalar('train/loss/average_regression', avg_reg_loss, epoch)
            writer.add_scalar('train/loss/average_classification', avg_clf_loss, epoch)
            writer.flush()
    return {
        'running_loss': running_loss,
        'running_regression_loss': running_regression_loss,
...
def test(loader, net, criterion, device):
    ...
    for _, data in tqdm(enumerate(loader),total=len(loader)):
        ...
        running_regression_loss += regression_loss.item()
        running_classification_loss += classification_loss.item()
    return running_loss / num, running_regression_loss / num, \
...
if __name__ == '__main__':
    ...
    for dataset_path in args.datasets:
        if args.dataset_type == 'voc':
            ...
            label_file = os.path.join('trained_models', "voc-model-labels.txt")
    ...
    extra_layers_lr = args.extra_layers_lr if args.extra_layers_lr is not \
                                              None else args.lr
    ...
    for epoch in range(last_epoch + 1, args.num_epochs):
        ...
        end_epoch = time()
        print(f"Epoch {epoch} took {end_epoch - start_epoch}")
        if epoch % args.validation_epochs == 0 or epoch == args.num_epochs - 1:
            ...
            writer.add_scalar('val/loss/average', val_loss, epoch)
            writer.add_scalar('val/loss/average_regression', val_regression_loss, epoch)

            writer.add_scalar(
                'val/loss/average_classification', val_classification_loss, epoch
            )
            writer.flush()
