...
class SSD(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for c in confidences:
            new_confidences.append(self.dequant(c))

        confidences = new_confidences

        new_locations = []
        for l in locations:
            new_locations.append(self.dequant(l))

        locations = new_locations
        #
        # for i,conf in enumerate(confidences):
        #     print(f"conf {i} has shape {conf.shape}")
        #
        #
        #
        # for i,loc in enumerate(locations):
        #     print(f"loc {i} has shape {loc.shape}")

        confidences = torch.cat(confidences, 1)
        locations = torch.cat(locations, 1)

        # print(f"Conf {confidence.shape}")
        # print(f"Loc {locations.shape}")
    ...
    def init_from_pretrained_ssd(self, model):
        ...
        state_dict = {
            k: v
            for k, v in state_dict.items() if not (
                k.startswith("classification_headers") or
                k.startswith("regression_headers")
            )
        }
    ...
    def fuse_model(self):
        for m in self.modules():
            ...
            if type(m) == InvertedResidual.InvertedResidual:
